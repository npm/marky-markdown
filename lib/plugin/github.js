...
function buildImageUrl (repository, url) {
  ...
  return prefix + path.join(repository.user, repository.repo, 'master', url.href)
}

function buildLinkUrl (repository, url) {
  return repository.https_url + path.join('/blob/master/', url.href)
}
  ...
  if (attr) {
    ...
    if (src) {
      ...
      if (!url.host && url.path && !url.path.match(/^\/\//)) {
        ...
        return attr[0] + quote + newUrl + quote + substring.slice(src[0].length + 1)
      }
    }
  }
