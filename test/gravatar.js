...
describe('gravatar', function () {
  ...
  it('replaces insecure gravatar img src URLs with secure HTTPS URLs', function () {
    ...
    assert.strictEqual($('img').length, 3)
    assert.strictEqual($('img').eq(0).attr('src'), 'https://secure.gravatar.com/avatar/123?s=50&d=retro')
  })
  ...
  it('leaves secure gravatar URLs untouched', function () {
    ...
    assert.strictEqual($('img').eq(1).attr('src'), 'https://secure.gravatar.com/avatar/456?s=50&d=retro')
  })

  it('leaves non-gravtar URLs untouched', function () {
    assert(~fixtures.gravatar.indexOf('http://not-gravatar.com/foo'))
    assert.strictEqual($('img').eq(2).attr('src'), 'http://not-gravatar.com/foo')
  })
})
